 ...
 *
 * OS data structure generated from application CortexM4
...
CONST(tpl_appmode_mask, OS_CONST) tpl_alarm_app_mode[ALARM_COUNT] = {
  1 /* alarm Task_Duty_Cycle_Update_Alarm : AppSTD */ ,
  1 /* alarm Task_High_Alarm : AppSTD */ ,
  0 /* alarm Task_Low_Alarm :  */ 
};
...
/* Task Task_Low identifier */
#define Task_Low_id 1
CONST(TaskType, AUTOMATIC) Task_Low = Task_Low_id;

/* Task Task_High identifier */
#define Task_High_id 2
CONST(TaskType, AUTOMATIC) Task_High = Task_High_id;

/* ISR EXTI0_CAT2 identifier */
#define EXTI0_CAT2_id 3
/*=============================================================================
 * Declaration of Alarm IDs
 */

/* Alarm Task_Duty_Cycle_Update_Alarm identifier */
#define Task_Duty_Cycle_Update_Alarm_id 0
CONST(AlarmType, AUTOMATIC) Task_Duty_Cycle_Update_Alarm = Task_Duty_Cycle_Update_Alarm_id;

/* Alarm Task_High_Alarm identifier */
#define Task_High_Alarm_id 1
CONST(AlarmType, AUTOMATIC) Task_High_Alarm = Task_High_Alarm_id;

/* Alarm Task_Low_Alarm identifier */
#define Task_Low_Alarm_id 2
CONST(AlarmType, AUTOMATIC) Task_Low_Alarm = Task_Low_Alarm_id;
...
#include "tpl_memmap.h"
VAR(tpl_stack_word, OS_APPL_DATA) Task1_stack_zone[3000/sizeof(tpl_stack_word)];
#define APP_Task_Task1_STOP_SEC_STACK
#include "tpl_memmap.h"

#define Task1_STACK {Task1_stack_zone, 3000}
...
/*-----------------------------------------------------------------------------
 * Task Task_Low descriptor
 */
#define APP_Task_Task_Low_START_SEC_CODE

#include "tpl_memmap.h"
/*
 * Task Task_Low function prototype
 */

FUNC(void, OS_APPL_CODE) Task_Low_function(void);
 ...
 * Task Task_Low stack
 */
#define APP_Task_Task_Low_START_SEC_STACK
#include "tpl_memmap.h"
VAR(tpl_stack_word, OS_APPL_DATA) Task_Low_stack_zone[3000/sizeof(tpl_stack_word)];
#define APP_Task_Task_Low_STOP_SEC_STACK
#include "tpl_memmap.h"

#define Task_Low_STACK {Task_Low_stack_zone, 3000}

/*
 * Task Task_Low context
 */
#define OS_START_SEC_VAR_NOINIT_32BIT
#include "tpl_memmap.h"
VAR(arm_core_context, OS_VAR) Task_Low_int_context;

#define Task_Low_CONTEXT &Task_Low_int_context
...
/*
 * Static descriptor of task Task_Low
 */
CONST(tpl_proc_static, OS_CONST) Task_Low_task_stat_desc = {
  /* context                  */  Task_Low_CONTEXT,
  /* stack                    */  Task_Low_STACK,
  /* entry point (function)   */  Task_Low_function,
  /* internal ressource       */  NULL,
  /* task id                  */  Task_Low_id,
#if WITH_OSAPPLICATION == YES
  /* OS application id        */  
...
#include "tpl_memmap.h"
/*
 * Dynamic descriptor of task Task_Low
 */
VAR(tpl_proc, OS_VAR) Task_Low_task_desc = {
  /* resources                      */  NULL,
#if WITH_OSAPPLICATION == YES
  /* if > 0 the process is trusted  */  0,    
#endif /* WITH_OSAPPLICATION */
...
/*-----------------------------------------------------------------------------
 * Task Task_High descriptor
 */
#define APP_Task_Task_High_START_SEC_CODE

#include "tpl_memmap.h"
/*
 * Task Task_High function prototype
 */

FUNC(void, OS_APPL_CODE) Task_High_function(void);
 ...
 */
/*
 * Task Task_High stack
 */
#define APP_Task_Task_High_START_SEC_STACK
#include "tpl_memmap.h"
VAR(tpl_stack_word, OS_APPL_DATA) Task_High_stack_zone[3000/sizeof(tpl_stack_word)];
#define APP_Task_Task_High_STOP_SEC_STACK
#include "tpl_memmap.h"

#define Task_High_STACK {Task_High_stack_zone, 3000}

/*
 * Task Task_High context
 */
#define OS_START_SEC_VAR_NOINIT_32BIT
#include "tpl_memmap.h"
VAR(arm_core_context, OS_VAR) Task_High_int_context;

#define Task_High_CONTEXT &Task_High_int_context
...
/*
 * Static descriptor of task Task_High
 */
CONST(tpl_proc_static, OS_CONST) Task_High_task_stat_desc = {
  /* context                  */  Task_High_CONTEXT,
  /* stack                    */  Task_High_STACK,
  /* entry point (function)   */  Task_High_function,
  /* internal ressource       */  NULL,
  /* task id                  */  Task_High_id,
#if WITH_OSAPPLICATION == YES
  /* OS application id        */  
#endif
  /* task base priority       */  3,
...
#include "tpl_memmap.h"
/*
 * Dynamic descriptor of task Task_High
 */
VAR(tpl_proc, OS_VAR) Task_High_task_desc = {
  /* resources                      */  NULL,
#if WITH_OSAPPLICATION == YES
  /* if > 0 the process is trusted  */  0,    
#endif /* WITH_OSAPPLICATION */
 ...
 */
/*-----------------------------------------------------------------------------
 * ISR EXTI0_CAT2 descriptor
 */
#define APP_ISR_EXTI0_CAT2_START_SEC_CODE
#include "tpl_memmap.h"
/*
 * ISR EXTI0_CAT2 function prototype
 */
FUNC(void, OS_APPL_CODE) EXTI0_CAT2_function(void);
#define APP_ISR_EXTI0_CAT2_STOP_SEC_CODE
#include "tpl_memmap.h"

/*-----------------------------------------------------------------------------
 * Target specific structures of isr EXTI0_CAT2
 */
/*
 * ISR EXTI0_CAT2 stack
 */
#define APP_ISR_EXTI0_CAT2_START_SEC_STACK
#include "tpl_memmap.h"
VAR(tpl_stack_word, OS_APPL_DATA) EXTI0_CAT2_stack_zone[200/sizeof(tpl_stack_word)];
#define APP_ISR_EXTI0_CAT2_STOP_SEC_STACK
#include "tpl_memmap.h"

#define EXTI0_CAT2_STACK {EXTI0_CAT2_stack_zone, 200}

/*
 * ISR EXTI0_CAT2 context
 */
#define OS_START_SEC_VAR_NOINIT_32BIT
#include "tpl_memmap.h"
VAR(arm_core_context, OS_VAR) EXTI0_CAT2_int_context;

#define EXTI0_CAT2_CONTEXT &EXTI0_CAT2_int_context
...
/*
 * Static descriptor of ISR EXTI0_CAT2
 */
CONST(tpl_proc_static, OS_CONST) EXTI0_CAT2_isr_stat_desc = {
  /* context                  */  EXTI0_CAT2_CONTEXT,
  /* stack                    */  EXTI0_CAT2_STACK,
  /* entry point (function)   */  EXTI0_CAT2_function,
  /* internal ressource       */  NULL,
  /* ISR id                   */  EXTI0_CAT2_id,
#if WITH_OSAPPLICATION == YES
  /* OS application id        */  
#endif
...
CONST(tpl_isr_static, OS_CONST) EXTI0_CAT2_isr_helper = {
  /* helper */ NULL,
  /* next */   NULL,
  /* id */     EXTI0_CAT2_id
};
...
#include "tpl_memmap.h"
/*
 * Dynamic descriptor of ISR EXTI0_CAT2
 */
VAR(tpl_proc, OS_VAR) EXTI0_CAT2_isr_desc = {
  /* resources                      */  NULL,
#if WITH_OSAPPLICATION == YES
  /* if > 0 the process is trusted  */  0,    
#endif /* WITH_OSAPPLICATION */
...
FUNC(void, OS_CODE) EXTI0_IRQ_Handler(void)
{
  ...
  if(EXTI->PR & EXTI_PR_PR0) {
     tpl_fast_central_interrupt_handler(EXTI0_CAT2_id);
...
tpl_stat_proc_table[TASK_COUNT+ISR_COUNT+1] = {
  ...
  &Task_Low_task_stat_desc,
  &Task_High_task_stat_desc,
  &EXTI0_CAT2_isr_stat_desc,
 ...
 */
/*-----------------------------------------------------------------------------
 * Alarm Task_Duty_Cycle_Update_Alarm descriptor
 *
...
CONST(tpl_task_activation_action, OS_CONST) Task_Duty_Cycle_Update_Alarm_action = {
  {
...
CONST(tpl_alarm_static, OS_CONST) Task_Duty_Cycle_Update_Alarm_static = {
  {
    /* pointer to counter           */  &SystemCounter_counter_desc,
    /* pointer to the expiration    */  tpl_raise_alarm
#if (WITH_ID == YES)
    /* id of the alarm for tracing  */  , Task_Duty_Cycle_Update_Alarm_id
#endif
#if WITH_OSAPPLICATION == YES
    /* OS application id            */  , 
#endif
  },
  /* action of the alarm  */  (tpl_action *)&Task_Duty_Cycle_Update_Alarm_action
};
...
VAR(tpl_time_obj, OS_VAR) Task_Duty_Cycle_Update_Alarm_alarm_desc = {
    /* pointer to the static part   */  (tpl_time_obj_static *)&Task_Duty_Cycle_Update_Alarm_static,
    /* next alarm                   */  NULL,
    /* prev alarm                   */  NULL,
    /* cycle                        */  300,
    /* date                         */  300,
...
/*-----------------------------------------------------------------------------
 * Alarm Task_High_Alarm descriptor
 *
 * This alarm does the activation of task Task_High
 */
...
CONST(tpl_task_activation_action, OS_CONST) Task_High_Alarm_action = {
  {
    /* action function  */  tpl_action_activate_task
  },
  /* task id            */  Task_High_id
...
CONST(tpl_alarm_static, OS_CONST) Task_High_Alarm_static = {
  {
    /* pointer to counter           */  &SystemCounter_counter_desc,
    /* pointer to the expiration    */  tpl_raise_alarm
#if (WITH_ID == YES)
    /* id of the alarm for tracing  */  , Task_High_Alarm_id
#endif
#if WITH_OSAPPLICATION == YES
    /* OS application id            */  , 
#endif
  },
  /* action of the alarm  */  (tpl_action *)&Task_High_Alarm_action
};
...
VAR(tpl_time_obj, OS_VAR) Task_High_Alarm_alarm_desc = {
    /* pointer to the static part   */  (tpl_time_obj_static *)&Task_High_Alarm_static,
    /* next alarm                   */  NULL,
    /* prev alarm                   */  NULL,
    /* cycle                        */  0,
    /* date                         */  50,
...
/*-----------------------------------------------------------------------------
 * Alarm Task_Low_Alarm descriptor
 *
 * This alarm does the activation of task Task_Low
 */
...
CONST(tpl_task_activation_action, OS_CONST) Task_Low_Alarm_action = {
  {
    /* action function  */  tpl_action_activate_task
  },
  /* task id            */  Task_Low_id
...
CONST(tpl_alarm_static, OS_CONST) Task_Low_Alarm_static = {
  {
    /* pointer to counter           */  &SystemCounter_counter_desc,
    /* pointer to the expiration    */  tpl_raise_alarm
#if (WITH_ID == YES)
    /* id of the alarm for tracing  */  , Task_Low_Alarm_id
#endif
#if WITH_OSAPPLICATION == YES
    /* OS application id            */  , 
#endif
  },
  /* action of the alarm  */  (tpl_action *)&Task_Low_Alarm_action
};
...
VAR(tpl_time_obj, OS_VAR) Task_Low_Alarm_alarm_desc = {
    /* pointer to the static part   */  (tpl_time_obj_static *)&Task_Low_Alarm_static,
    /* next alarm                   */  NULL,
    /* prev alarm                   */  NULL,
    /* cycle                        */  0,
    /* date                         */  0,
...
CONSTP2VAR(tpl_time_obj, AUTOMATIC, OS_APPL_DATA)
  ...
  &Task_Duty_Cycle_Update_Alarm_alarm_desc,
  &Task_High_Alarm_alarm_desc,
  &Task_Low_Alarm_alarm_desc
};
#define OS_STOP_SEC_CONST_UNSPECIFIED
#include "tpl_memmap.h"
